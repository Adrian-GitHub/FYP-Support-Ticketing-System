...
const CloseAbandonedTicket = async(req, res) => {
    ...
    // Let Camunda know about current action
    // Construct the payload
    const requestBody =  {
        "variables": {
...
const OpenTicketOnBehalf = async(req, res) => {
        ...
        .then(res => res.json())
        .then(json => proc_Camunda_ID = json.id).catch((err) => console.log(err));
        ...
        await fetch(`http://localhost:8080/engine-rest/task?processInstanceId=${proc_Camunda_ID}`, {
            method: 'GET',
            headers: { 'Content-Type': 'application/json' },
        })
        .then(res => res.json())
        ...
        const newTicket = new Ticket({camundaID: proc_Camunda_ID, title: req.body.ticketTitle, desc: req.body.description, createdBy: req.body.onBehalf, createdById: user_id , currentSupportStaff: username, staffId: user_id, status: 2});
        ...
        // Now the ticket is assigned to staff but the BPM Engine doesn't know about it. Let's notify it again to update the process instance
        const requestBody_2 =  {
            "variables": {
            "cancel": {"value":"false","type":"String"} // we don't want to cancel the ticket
            }
        ...
        await fetch(`http://localhost:8080/engine-rest/task/${Camunda_ID}/complete`, {
            method: 'POST',
            body: JSON.stringify(requestBody_2),  //client
            headers: { 'Content-Type': 'application/json' },
        })
        .catch();
