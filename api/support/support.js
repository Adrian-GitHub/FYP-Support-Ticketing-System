...
// Load Support Staff's model for MongoDB(used by Mongoose)
const Support = require("../../database/models/Support/Support_Account");
// MongoDB
...
router.post("/Register", (req, res) => { // Form validation
        connection.collection('users').findOne({ username: req.body.username }).then(user => {
            if (user) {
                ...
                // Create new user
                const newUser = new Support({ name: req.body.name, username: req.body.username, password: hashedPassword, salt: salt });
